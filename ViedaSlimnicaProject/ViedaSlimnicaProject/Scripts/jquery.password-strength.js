...
(function($) {
    ...
    var password_Strength = new function() {
        ...
        this.getStrengthInfo = function(passwordVal) {
            ...
            //check space
            if (spaces > 0) {
                return "Parole nedrīkst saturēt atstarpes!";
            }

            //invalid characters
            if (inValidChars !== '') {
                return "Neatļauti simboli: " + inValidChars;
            }

            //max length
            if (len > password_settings.maxLength) {
                return "Parole pārāk gara!";
            }

            //GET NUMBER OF CHARACTERS left
            if ((specials + uppers + nums + lowers) < password_settings.minLength) {
                msg += "Vajadzīgi vēl " + (password_settings.minLength - (specials + uppers + nums + lowers)) + " simboli. ";
            }
            ...
            if (nums >= password_settings.numberLength) {
                ...
                msg += (password_settings.numberLength - nums) + " cipari, ";
            }
            ...
            if (specials >= password_settings.specialLength) {
                ...
                msg += (password_settings.specialLength - specials) + " simboli, ";
            }
            ...
            if (uppers >= password_settings.upperLength) {
                ...
                msg += (password_settings.upperLength - uppers) + " lielie burti, ";
            }
    ...
    $.fn.password_strength = function(options) {
        ...
        $.ajax({
            type: "GET",
            url: "http://localhost:13770/Content/xml/PasswordPolicy.xml", //use absolute link if possible
            dataType: "xml",
        ...
        return this.each(function() {
            ...
            //bar position
            var control = $("[id='" + this.id + "']");
            var fromLeft = control.position().left;
            var width = control.width();
            var padding = parseInt(control.css('padding-right'));
            var barLeftPos = fromLeft + width + (padding * 2);
